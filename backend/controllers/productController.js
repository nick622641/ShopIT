...
exports.newProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    const { name, price, width, height, datePublished, categoryOne, categoryTwo, categoryThree, description } = req.body
    ...
    if( !categoryOne    ) { return next( new ErrorHandler( `Please select a ${process.env.CATEGORY_ONE}` , 400 )) }
    // if( !categoryTwo    ) { return next( new ErrorHandler( `Please select a ${process.env.CATEGORY_TWO}` , 400 )) }
    // if( !categoryThree  ) { return next( new ErrorHandler( `Please select a ${process.env.CATEGORY_THREE}` , 400 )) }    
